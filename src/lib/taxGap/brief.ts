...
export function buildBrief(profile: Profile, rules: TaxYearRules = rulesFor(profile.year)): string {
  ...
  return [
    ...
    `- **เงินเหลือใช้เฉลี่ยเดือนละ ${money(score.monthlyLeftAfter)} บาท** ตามแผนปัจจุบัน (ก่อนกันเงินคือ ${money(score.monthlyLeftBefore)} บาท) ทุกบาทที่แนะนำให้ซื้อเพิ่มจะกินตัวเลขนี้ลงไปอีก ก่อนเสนอให้ซื้อเต็มเพดาน ให้บอกด้วยว่าเงินเหลือใช้ต่อเดือนจะเหลือเท่าไหร่ และถามว่ารับไหวไหม`,
