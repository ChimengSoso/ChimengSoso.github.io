...
- Pushing to `master` triggers `.github/workflows/deploy.yml`, which runs `withastro/action@v3` (installs deps, runs `astro build`) and deploys `dist/` to GitHub Pages via `actions/deploy-pages@v4`. There is no separate staging environment — every push to `master` goes live immediately.
