...
- **`src/data/socialLinks.js`** is the single source of truth for social profile URLs + their inline-SVG icon path data, consumed by `index.astro` (profile icons and the JSON-LD `sameAs` list) and `Layout.astro` (site footer, via the filtered `footerSocialLinks` export). All icons site-wide are inline SVGs — the Font Awesome CDN stylesheet was deliberately removed (it was render-blocking external CSS on every knowledge page); don't reintroduce an icon font for new icons.
