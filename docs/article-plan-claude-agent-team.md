...
```ts
{
  ...
  title: "ตั้งบริษัทซอฟต์แวร์ที่ไม่มีพนักงานเป็นคนสักคน",   // ← เปลี่ยนจากชื่อเดิม (2026-07-16, ฟีดแบ็ก Chi): ชื่อเดิมสปอยกลไก dev/QA ตั้งแต่หัวเรื่อง
  desc: "เปิดบริษัทซอฟต์แวร์ของตัวเอง แต่ไม่ต้องจ้างพนักงานสักคน — ทีมที่มีหัวหน้าแบ่งงาน มีลูปแก้งาน และเด้งกลับมาถามเราเมื่อโจทย์ไม่ชัด คือของจริงที่ทำได้วันนี้แค่ไหน",
  tag: "Claude Code",
  soon: false,
...
- ภาษาไทย เป็นกันเอง "เข้าใจง่าย" — อ่าน `claude-intro.astro` / `ai-taste.astro` ก่อนเขียน (และ CLAUDE.local.md ถ้ามีในเครื่อง — สไตล์ไกด์เต็มของซีรีส์)
...
### h2: ลองสวมบทเป็นเจ้าของบริษัทซอฟต์แวร์ดูสักตั้ง
> **แก้ตามฟีดแบ็กของ Chi (2026-07-16):** ห้ามเปิดบทความด้วยภาพ "AI หลายตัวคุยกันในห้องแชต" ตรง ๆ เพราะสปอยกลไก
> dev/QA ทั้งหมดตั้งแต่ย่อหน้าแรก — ให้เปิดด้วยฉากที่ **ใหญ่และคลุมเครือกว่า** ก่อน: ชวนผู้อ่านสวมบทเป็น
> **เจ้าของบริษัทซอฟต์แวร์ที่เพิ่งเริ่มต้น** (ยังไม่มีพนักงาน ทำเองทุกอย่าง อยากมีทีม) แล้ว**ค่อยเฉลยทีหลัง**ในหัวข้อถัดไปว่า
> ทีมที่ว่านี้คือ Claude agent — เก็บความรู้สึก "น่าค้นหา" ไว้ให้นานกว่าเดิม อย่ารีบบอกว่าเป็น AI ตั้งแต่ต้น
- เล่าฉาก: เปิดบริษัทพัฒนาซอฟต์แวร์ของตัวเอง เริ่มจากศูนย์ ทำทุกอย่างคนเดียว (คิดสเปก/เขียน/ตรวจ/ส่งงาน) แล้วอยากมีทีม
  (คนรับสเปก → คนเขียน → คนตรวจ) แต่ยังไม่บอกว่าทีมนั้นคือ AI — ปิดท้ายด้วยประโยคชวนสงสัยแทนการเฉลย
  ("ตั้งทีมแบบนี้ได้ตั้งแต่วันนี้เลย โดยไม่ต้องจ้างใครสักคน")
- ตัดโทนยืนยันความเป็นไปได้ทันที ("ผมลองเอาโจทย์จริงไปให้ทีมแบบนี้ทำดู แล้วมันก็ทำได้จริง") ออกจาก h2 แรก — ย้ายไปพูดถึงหลัง h2 ถัดไป (แกะศัพท์) ที่เฉลยแล้วว่าเป็น Claude agent
- callout-note "บทความนี้จะแยกให้ชัดว่าอะไรทำได้จริง อะไรต้องประกอบเอง" ก็ตัดออกจาก h2 แรกด้วยเหตุผลเดียวกัน (สปอยก่อนเวลา) — ถ้าจะใส่ ย้ายไปหลังเฉลยแล้ว
- อ้างอิง memory `no-premature-reveal` — หลักการเดียวกับที่ใช้กับประโยคเดี่ยว ๆ ตอนนี้ขยายมาใช้กับโครงทั้ง h2 แรกของบทความด้วย

### h2: แกะศัพท์ก่อน
- ตรงนี้แหละคือจุดที่เฉลยว่าทีมที่เพิ่งเล่าไปคือ Claude agent — เปิดหัวข้อด้วยการเฉลยสั้น ๆ ก่อนแกะศัพท์
- `agent` — *"เอเจนต์"* = ตัวแทน/ผู้ทำงานแทน (Claude 1 ตัว = พนักงาน 1 คน) — ยังไม่เคยแกะในซีรีส์ (เช็คแล้ว) แกะได้เต็มที่
- `orchestrate` — **ห้ามแกะซ้ำ** เช็คแล้วว่า context-pollution.astro แกะ `orchestrator` ไปแล้วครบ (คำอ่าน "ออร์เคสเทรเตอร์" + ที่มา orchestra/วาทยกร) — ให้อ้างอิงกลับบทนั้นสั้นๆ แทน (ลิงก์ `../context-pollution/`)
- `subagent`, `workflow` — สั้นๆ (เช็ค grep ก่อนแกะเช่นกัน)
- **building block:** `card` รวมศัพท์ · **grep src/pages/knowledge/*.astro หาทุกคำก่อนแกะ** ไม่ใช่แค่ agent
...
## 5. Fact-check ก่อนเริ่มเขียนเนื้อหา (WebSearch)

**ทำรอบนี้ให้จบก่อนลงมือเขียน h2 ไหนๆ** ไม่ใช่ทิ้งไว้ทำตอนท้าย — ถ้าฟีเจอร์เปลี่ยนชื่อ/เลิก experimental
หลังเขียนไปแล้ว section "3 วิธีจัดทีม" ทั้งก้อนต้องรื้อใหม่ ยืนยันตัวเลข/ชื่อฟีเจอร์ให้เป๊ะ (เจ้าของเคยจับผิดพลาดมาก่อน):
...
- Artifact URL (ของเจ้าของ, **private — ห้ามฝังลิงก์นี้ตรงๆ ในบทความ** ผู้อ่านทั่วไปเปิดไม่ได้): `https://claude.ai/code/artifact/fa4954b8-cc69-435f-83cc-96ed040d22ed`
- ไฟล์ต้นฉบับเก็บถาวรแล้วที่ `docs/samples/team-room.html` (ย้ายออกจาก scratchpad แล้ว — เปิดดูตรงนี้ได้เลย ไม่ต้องไปหาใน scratchpad)
- **วิธีใช้ในบทความ:** ถ่าย screenshot จากไฟล์นี้ เก็บใน `public/knowledge/` แล้วอ้างอิงแบบรูปปกติ (ดูกฎรูปภาพใน CLAUDE.md) — ถ้าไม่ชัวร์ว่า Chi อยากได้แบบไหน ให้ถามก่อนแทรก
