...
## 10. [ ] Finish bringing `/cp/archery` up to `/cp/pick-books` standard
...
**Still to do, roughly in value order.**
1. **A picture for part 5 (`เปลี่ยนไปนับการไหล`).** This is the page's biggest conceptual jump, from walking the board to a closed form, and it currently has none. Draw the targets as columns holding `n_i(0)`, one strong archer leaving per round, and the diagonal path whose sum is the range sum the code actually computes. The model already has everything needed.
2. **A picture for part 6 (`สองเฟส`).** A timeline: phase one is linear and ends at `i₀ − 1` where `i₀` is the leftmost target holding a strong archer, then phase two is cyclic. The `ระวัง` callout there explains why the single formula goes negative; the picture should show the moment the rule changes.
3. **Meaning-first bullets for `texBoard` in part 1.** The three-branch recurrence is currently explained in one dense paragraph. `CLAUDE.md` asks for an anchor sentence, one bullet per branch, and a closing consequence line; `pick-books`'s `g[k]` passage is the reference shape.
4. **Sub-headings.** `archery` has fifteen `h2` and zero `h3`, while `pick-books` uses eight `h3` to break up its long sections. Parts 5, 6 and 7 are long enough to need them; keep any new `h3` smaller than the layout's `h2` (see the heading-size rule in `CLAUDE.md`).
5. **A fourth minigame set.** There are three (`AR_PUZZLES`): the two statement examples plus one where every start ties. The set that is missing is the one part 6 calls the real obstacle, where target 1 starts with no strong archer, so phase one actually matters. Every set's answer is already cross-checked against a full simulation at build time, so add it the same way.
6. **A closing line on what transfers.** The page currently ends on a note about two bugs the checker caught. That is good material but it is not something the reader carries to another problem. This is the `archery` half of item 9's Item B.
