...
## 13. [x] Seven new programming.in.th problems (2026-09-09)

Added `hedgehog-zigzag`, `grasshopper-field`, `mole-tunnels`, `oil-blocks`, `convention-centre`, `atm-robbery` and `range-subarrays`, one commit each, taking the vault from 29 to 36 pages and the programming.in.th group from 18 to 25. Two topics were added to `CP_TOPICS` on purpose: `binary lifting` and `scc`.

Every page follows the standing `/cp/` contract: a playable round before the reveal, a derivation callout as the first child inside the solution's `<Spoiler>`, all figures computed in the frontmatter, and a build-time assertion that throws when the taught method disagrees with an independent brute force. Three pages ship a real bug story with the minimal failing input found by exhaustive sweep: `oil-blocks` (two of the six shapes started their cut at `2K` instead of `K`, which still answers the statement's sample correctly and fails on a 2x2 grid), `grasshopper-field` (inserting into the top-four tables before the equal-petal group finished, which fails on a 3x3 field where every flower has one petal), and the same page's proof that keeping three candidates instead of four is genuinely not enough.

**Verified:** `check` + `lint` + `build` clean; every code block extracted back out of `dist/` and recompiled, then raced against its brute force (700, 400, 400, 903, 900, 759 and 1,500 random cases respectively) with the quoted timings re-measured from those same binaries; zero em-dashes and zero zero-width spaces in all seven; the centred-axis walk clean on all seven; game boards keep their cell size at 375px and scroll inside their own card; every step player holds a constant width across all its steps; prev/next chains correctly through the seven and back to `flood-walls`; OG cards generated; all seven `noindex` and absent from the sitemap; the hub shows 36 cards with the two new filter chips and updated counts.

**Two bugs the owner spotted on review (fixed, `b0f4bae`).** (1) The `io-note` under the INPUT/OUTPUT table was wrapped in `<Spoiler linked>` on seven pages, and a buttonless spoiler can only be unlocked by a button that precedes it as a sibling in the same section, so those notes were blurred permanently no matter what the reader opened. Four of them genuinely leak the answer and now carry their own button; three do not and are simply unblurred. (2) On eleven pages, scoped CSS aimed at nodes built with `document.createElement` was dropped at build, because Astro puts the scope attribute on every compound of the selector. The earlier repair pass had globalised only the class-bearing descendants, so the plain `th`/`td`/`b` rules stayed broken and those game tables rendered with no borders or padding at all; `/cp/prefix-sums`'s newest game was fully unstyled. Both classes are now covered by a one-pass browser audit written up in `CLAUDE.md`, which reports zero hits across all 36 pages.
